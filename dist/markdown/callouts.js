...
let md = null;
export function setCalloutMarkdownParser(parser) {
    md = parser;
}
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line === ":::" && inCallout) {
            ...
            if (config) {
                ...
                result.push(`<div class="callout ${config.class}">`, `<div class="callout-title"><span class="callout-icon">${config.icon}</span><span>${calloutTitle}</span></div>`, `<div class="callout-content">${parsedBody}</div>`, `</div>`);
            }
            inCallout = false;
            continue;
        }
