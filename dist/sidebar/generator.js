...
function mergeSidebars(configItems, autoItems) {
    ...
    for (const configItem of configItems) {
        if (configItem.items && configItem.items.length > 0) {
            const autoCategory = autoItems.find(a => a.items && normalizeLabel(a.label) === normalizeLabel(configItem.label));
    ...
    for (const autoItem of autoItems) {
        if (autoItem.items && autoItem.items.length > 0) {
            const existsInResult = result.some(r => r.items && normalizeLabel(r.label) === normalizeLabel(autoItem.label));
            if (!existsInResult) {
                result.push({
                    ...autoItem,
                    autoGenerated: true,
